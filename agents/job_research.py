...
class JobResearchAgent:
    """Role discovery and evidence matching over a supplied job corpus."""
    ...
    def __init__(self, jobs_path: str | Path, profile_text: str, trace_dir: str = "traces") -> None:
        self.jobs_path = Path(jobs_path)
        self.profile_text = profile_text
        self.logger = TraceLogger("role-discovery", trace_dir)
    ...
    def _terms(text: str) -> list[str]:
        """Normalize lexical intent without treating unrelated phrases as equivalent matches."""
    ...
    def _intent_relevance(self, query: str, job: JobCandidate) -> float:
        ...
        corpus = [
            set(self._terms(f"{item.title} {item.description}"))
    ...
    def discover(self, query: str, top_k: int = 5) -> list[JobCandidate]:
        with traced_step(self.logger, "discover", {"query": query, "top_k": top_k}) as trace:
            ...
            results = [job for _, job in sorted(scored, key=lambda x: x[0], reverse=True)[:top_k]]
            trace.set_output([asdict(job) for job in results])
            return results

    def analyze(self, job: JobCandidate) -> JobMatch:
        with traced_step(self.logger, "extract_profile_evidence", job.id) as trace:
            profile_lower = self.profile_text.lower()
            job_lower = job.description.lower()
            matched = [s for s in self.SIGNALS if s in profile_lower and s in job_lower]
            gaps = [s for s in self.SIGNALS if s in job_lower and s not in profile_lower]
        ...
        with traced_step(self.logger, "match_rationale", {"job_id": job.id, "score": score}) as trace:
            if self.llm.live:
                rationale = self.llm.complete(
                    system="Compare the role with the supplied profile. Use only explicit profile evidence and preserve gaps.",
                    prompt=f"""Role:\n{job.title} at {job.company}\n{job.description}\n\nProfile:\n{self.profile_text}\n\nMatched signals: {matched}\nGaps: {gaps}\nGive a concise rationale that separates evidence from gaps.""",
                ).text
            else:
    ...
    def native_tool_demo(self, query: str) -> dict[str, Any]:
        def search_jobs(args: dict[str, Any]) -> Any:
            ...
            return [asdict(job) for job in self.discover(q, top_k=k)]
        ...
        runtime = AnthropicToolRuntime(
            tools=[
                Tool(
                    name="search_jobs",
                    description="Search the local role corpus for records relevant to a role intent.",
                    input_schema={
                        "type": "object",
                        "properties": {
                            "query": {"type": "string"},
                ...
                Tool(
                    name="analyze_job",
                    description="Compare one discovered role with explicit profile evidence and return matched signals and gaps.",
                    input_schema={
                        "type": "object",
            ...
            system=(
                "Use the tools to find relevant roles and compare them with the supplied profile. "
                "Do not invent role or profile facts, and keep gaps explicit."
            ),
        )
        with traced_step(self.logger, "native_claude_tool_loop", {"query": query}) as trace:
            result = runtime.run(
                f"Find the role that best matches this intent: {query}. Inspect multiple records and explain the choice."
            )
